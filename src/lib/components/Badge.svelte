<script lang="ts">
  ...
  const tones: Record<string, string> = {
    neutral: "bg-white/[0.04] text-[var(--color-muted)] border-[var(--color-border-soft)]",
...
<span class="inline-flex items-center rounded-lg border px-2 py-0.5 text-xs font-medium {tones[tone]}">
  {@render children()}
</span>
