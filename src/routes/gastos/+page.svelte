<script lang="ts">
  ...
  async function syncWhatsApp() { if (syncingWhatsApp) return; syncingWhatsApp = true; try { await api.syncWhatsAppExpense("invoice"); showToast("Documento leído desde WhatsApp y añadido a revisión como factura"); await load(); } catch (error) { showToast(error instanceof Error ? error.message : "No se pudo leer WhatsApp", "err"); } finally { syncingWhatsApp = false; } }
...
<section class="workspace-page space-y-6">
  <div class="flex flex-wrap items-end justify-between gap-4"><div><p class="workspace-index">GASTOS Y FACTURAS</p><h1 class="text-3xl font-semibold text-[var(--color-text)]">Bandeja de facturas recibidas</h1><p class="mt-1 text-sm text-[var(--color-muted)]">En local, Comprobar WhatsApp lee el último documento y lo guarda como factura para que puedas corregirlo durante la revisión.</p></div><div class="flex flex-wrap gap-2"><Button variant="secondary" disabled={syncingWhatsApp} onclick={syncWhatsApp}>{syncingWhatsApp ? "Leyendo WhatsApp…" : "Comprobar WhatsApp"}</Button><Button variant="primary" onclick={reset}>+ Nueva factura</Button></div></div>
