...
<section class="workspace-page space-y-6">
  <div class="flex flex-wrap items-end justify-between gap-4"><div><p class="workspace-index">GASTOS Y FACTURAS</p><h1 class="text-3xl font-semibold text-[var(--color-text)]">Bandeja de facturas recibidas</h1><p class="mt-1 text-sm text-[var(--color-muted)]">Sube una factura o ticket, revísalo y apruébalo antes de incorporarlo a Caja.</p></div><div class="flex flex-wrap gap-2"><Button variant="secondary" disabled={syncingWhatsApp} onclick={syncWhatsApp}>{syncingWhatsApp ? "Leyendo WhatsApp…" : "Leer WhatsApp"}</Button><Button variant="primary" onclick={reset}>+ Nueva factura</Button></div></div>
